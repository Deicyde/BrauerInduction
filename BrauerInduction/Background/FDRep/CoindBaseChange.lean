import BrauerInduction.Background.FDRep.Induced
import BrauerInduction.Background.FDRep.CoindBasic
...
private noncomputable def comapCoindToCoindComap
    [Finite G]
    (K : Subgroup G) [K.Normal]
    (Q : Subgroup (G ⧸ K))
    (σ : FDRep k Q) :
...
private noncomputable def coindComapToComapCoind
    [Finite G]
    (K : Subgroup G) [K.Normal]
    (Q : Subgroup (G ⧸ K))
    (σ : FDRep k Q) :
...
private noncomputable def comap_coind_quotient_linearEquiv
    [Finite G]
    (K : Subgroup G) [K.Normal]
    (Q : Subgroup (G ⧸ K))
    (σ : FDRep k Q) :
